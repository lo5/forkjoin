...
(function() {
  var _getProperty, async, collect, createTask, forEach, fork, forkjoin, get, getProperty, isFunction, isFuture, join, lift, map, resolve, seq,
    slice = [].slice;
  ...
  map = function(array, createFuture) {
    var element;
    return collect((function() {
      ...
      for (j = 0, len = array.length; j < len; j++) {
        element = array[j];
        results1.push(createFuture(element));
      }
      return results1;
    })());
  };

  forEach = function(array, createFuture) {
    var element;
    return seq((function() {
      var j, len, results1;
      results1 = [];
  ...
  _getProperty = function(obj, attributes) {
    ...
    if (obj) {
      if (attributes.length) {
        ...
        if (attributes.length) {
          return getProperty(property, attributes);
        } else {
          return property;
        }
      } else {
  ...
  getProperty = function(obj, attributes) {
    return _getProperty(obj, attributes.slice(0));
  };

  get = function() {
    var attributes, obj;
    obj = arguments[0], attributes = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    if (isFuture(obj)) {
      return fork(function(go) {
        return obj(function(error, result) {
          if (error) {
            return go(error);
          } else {
            return go(null, getProperty(result, attributes));
          }
        });
      });
    } else {
      return fork(function(go) {
        return go(null, getProperty(obj, attributes));
      });
    }
  };
